...
def fill_receipt_sender_params(data):
    receipt_book = openpyxl.load_workbook(FILES_PATH + "blank_receipt_pattern.xlsx")
    ...
    receipt_book.save(FILES_PATH + "receipt_pattern.xlsx")

    # with open("blank_receipt_pattern.xlsx", 'rb') as receipt_file:
    #     content = receipt_file.read()

    convertapi.api_secret = 'yyKssjcTKewwZvnx'
    res = convertapi.convert('pdf', {"File": FILES_PATH + 'receipt_pattern.xlsx'}, from_format='xlsx')
    res.file.save(FILES_PATH)

    with open(FILES_PATH + 'receipt_pattern.xlsx', 'rb') as receipt_file:
        content = receipt_file.read()
...
async def fill_and_send_personal_receipts(payers_data, qr_pattern, server, email, num=None):
    ...
    for payer in payers_data:
        ...
        pdfmetrics.registerFont(TTFont('Arial', FONTS_PATH + 'Arial Cyr Regular.ttf'))
        can.setFont('Arial', 9)
        ...
        for _ in range(2):
            ...
            for section in center(
                    f"ЛС: {payer[0].strip()}; ФИО обучающегося: {payer[1].strip()}               "
                    f"; Назначение: {payer[2].strip()} ;КБК: {payer[3].strip()}; "
                    f"ОКТМО: {payer[4]}",
                    87).split('\n'):
        ...
        can.drawCentredString(353, 687, f"Сумма: {get_amount(payer[7])}")
        # добавляем второй текст (Сумма оплаты)
        can.drawCentredString(353, 504, f"Сумма: {get_amount(payer[7])}")

        # создаем QR-код по данным плательщика и добавляем его на холст
        qrdt = make_qr_data(qr_pattern, payers_data[0])
        print(qrdt)
        make_qr(qrdt)
        can.drawInlineImage(FILES_PATH + "qr_code.png", 40, 485, 120, 120, anchor='u')
        ...
        can.showPage()
        existing_pdf = PdfFileReader(open(FILES_PATH + "receipt_pattern.pdf", "rb"))
        ...
        output.addPage(page)
        output_stream = open(FILES_PATH + f"dest_{num}.pdf", "wb")
        output.write(output_stream)
        output_stream.close()

        await send_email(email, "Квитанция", "Оплата", FILES_PATH + f"dest_{num}.pdf", server)
        os.remove(FILES_PATH + f"dest_{num}.pdf")
        # break
